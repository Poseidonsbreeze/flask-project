...
export default function Dashboard({ user, onNav, toast }) {
  ...
  async function seed() {
    ...
    try {
      ...
      toast(
        `${d.message} — ${d.total_added} new scholarships added`,
        "success",
      );
    ...
    <div style={S.page}>
      ...
      <div style={S.welcome}>
        <div>
          ...
          <p style={S.welcomeSub}>
            {scholarships.length} scholarships in database · {matches.length}{" "}
      ...
      <div style={S.statsGrid}>
        {[
          ...
          {
            label: "Top Score",
            val: topM[0] ? `${Math.round(topM[0].score)}%` : "—",
            Icon: TrendingUp,
            color: "var(--gold)",
            page: "matches",
          },
          ...
          <div
            key={label}
            style={{ ...S.stat, animationDelay: `${i * 60}ms` }}
            onClick={() => onNav(page)}
          >
            <div
              style={{
      ...
      <div style={S.twoCol}>
        ...
        <div style={S.panel}>
          <div style={S.panelHead}>
            ...
            <button style={S.panelLink} onClick={() => onNav("matches")}>
              View all
            </button>
          </div>
          {topM.length === 0 ? (
            ...
            topM.map((m, i) => (
              <div
                key={m.id || `${m.title}-${i}`}
                style={{ ...S.matchRow, animationDelay: `${i * 50}ms` }}
              >
        ...
        <div style={S.panel}>
          <div style={S.panelHead}>
            ...
            <button style={S.panelLink} onClick={() => onNav("scholarships")}>
              Browse
            </button>
          </div>
          {nearest.length === 0 ? (
            ...
            nearest.map((s, i) => {
              ...
              return (
                <div
                  key={s.id}
                  style={{ ...S.deadlineRow, animationDelay: `${i * 50}ms` }}
                >
                  <div
                    style={{
                      ...S.deadlineDays,
        ...
        <div style={S.nudge}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--ink-2)" }}>
              Add a profile text describing your background and interests so the
              matching engine can find your best scholarships.
            </div>
          </div>
          <button style={S.nudgeBtn} onClick={() => onNav("profile")}>
            Go to Profile →
          </button>
        </div>
      )}
...
const S = {
  ...
  welcome: {
    ...
    backgroundImage:
      "radial-gradient(ellipse at top right, rgba(52,211,153,.05) 0%, transparent 60%)",
